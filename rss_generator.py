...
RSSdefinitions.append(['avalikteenistus',   'avalikteenistus',  'Avalik teenistus', 'töökohad',         'http://www.rahandusministeerium.ee',   'http://www.rahandusministeerium.ee/et/avalikud-konkursid?page=' + str(random.randint(1, 12))])  # noqa E241
RSSdefinitions.append(['bns',               'bns',              'BNS',              'uudised',          'http://bns.ee',                        ''])  # noqa E241
RSSdefinitions.append(['err',               'err',              'ERR',              'videoarhiiv',      'http://arhiiv.err.ee',                 'https://arhiiv.err.ee/viimati-lisatud/err-videoarhiiv/koik'])  # noqa E241
...
RSSdefinitions.append(['mixcloud',          'paranoidtxt',      'paranoidtxt ',     '@mixcloud',        'https://www.mixcloud.com',             'http://www.mixcloud.com/paranoidtxt/uploads/?order=latest'])  # noqa E241
RSSdefinitions.append(['mixcloud',          'rhythmdr',         'rhythmdr ',        '@mixcloud',        'https://www.mixcloud.com',             'http://www.mixcloud.com/rhythmdr/uploads/?order=latest'])  # noqa E241
RSSdefinitions.append(['nommeraadio',       'nommeraadio',      'Nõmme Raadio',     'uudised',          'http://www.nommeraadio.ee',            ''])  # noqa E241
RSSdefinitions.append(['osta',              'ostaee',           'otsa.ee',          'pakkumised',       'https://www.osta.ee',                  'http://www.osta.ee/index.php?fuseaction=search.search&orderby=enda&q[cat]=1397&q[location]=Tartu&start=' + str(random.randint(0, 25)*60)])  # noqa E241
RSSdefinitions.append(['phpbb',             'arutelud',         'Arutelud',         'foorumipostitused','http://arutelud.com',                  'http://arutelud.com/search.php?search_id=active_topics'])  # noqa E241
RSSdefinitions.append(['phpbb',             'cbfoorum',         'CB foorum',        'foorumipostitused','http://foorum.cbradio.ee',             'http://foorum.cbradio.ee/search.php?search_id=active_topics'])  # noqa E241
RSSdefinitions.append(['phpbb',             'isikee',           'isik.ee',          'foorumipostitused','http://www.isik.ee/foorum',            'http://www.isik.ee/foorum/search.php?search_id=active_topics'])  # noqa E241
...
RSSdefinitions.append(['stokker',           'stokker',          'Stokker',          'outlet pakkumised','http://www.stokker.ee',                'http://www.stokker.ee/kampaaniad/tooriistade-outlet?page=' + str(random.randint(1, 15)) + '&limit='+ str(maxArticleCount) +'&instorage=1'])  # noqa E241 &orderby=price&orderdir=asc
...
RSSdefinitions.append(['trm',               'tooriistamarket',  'Tööriistamarket',  'head pakkumised',  'http://www.tooriistamarket.ee',        'http://www.tooriistamarket.ee/et/head-pakkumised?price=1&page=' + str(random.randint(0, 20))])  # noqa E241
...
for i in range(1, len(sys.argv)):
    ...
    if str(curSisendArg) == "-limit":
        ...
        rss_print.printDebugMessageLevel = max(rss_print.printDebugMessageLevel, 5)
        rss_print.print_debug(__file__, 'sisend: "' + curSisend + '"', 0)
    elif str(curSisend) != "":
...
for curRSS in RSStoGenerate:
    ...
    try:
        ...
        rss_print.print_debug(__file__, parsers_common.elemtreeToString(pageHtmlTree), 5)
    ...
    articleDataDict = curParser.getArticleListsFromHtml(articleDataDictEmpty, pageHtmlTree, curDomain, maxArticleCount, getArticleBodies)
    rss_print.print_debug(__file__, "articleDataDict = " + str(articleDataDict), 3)

    lastCountValue = 0
    for x in articleDataDict:
        rss_print.print_debug(__file__, str(x) + "=", 3)
        rss_print.print_debug(__file__, articleDataDict[x], 3)
        curCountValue = len(articleDataDict[x])
        ...
        elif str(x) == "urls" and curCountValue == 0:
