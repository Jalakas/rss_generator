...
def fill_article_dict(articleDataDict, pageTree, domain):
    ...
    for i in parsers_common.article_urls_range(articleDataDict["urls"]):
        if parsers_common.should_get_article_body(i):
            ...
            # author
            curArtAuthor = parsers_common.xpath_to("single", pageTree, '//div[@class="row pb-3"]/div[1]/div[@class="mt-1 text-uppercase"]/small/text()')
            ...
            if not curArtDesc:
                curArtDesc = parsers_common.xpath_to("single", pageTree, '//div[@class="container mb-5"]', parent=True)
            articleDataDict["descriptions"] = parsers_common.list_add_or_assign(articleDataDict["descriptions"], i, curArtDesc)

            # image
            curArtImg = parsers_common.xpath_to("single", pageTree, '//div[@class="row"]/div/figure[1]/a[@data-lightbox="treimages"][1]/img/@src')
